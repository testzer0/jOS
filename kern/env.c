...
env_setup_vm(struct Env *e)
{
	...
	for (i = 0; i < UTOP/PTSIZE; i++) {
		e->env_pgdir[i] = 0; 						// empty
	}
	for (i = UTOP/PTSIZE; i < NPDENTRIES; i++) {
		e->env_pgdir[i] = kern_pgdir[i]; 			// reuse mappings
	}
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	uint32_t real_len = end - start;

	while (start < end) {
		struct PageInfo *pp = page_alloc(0);
		if (!pp) 
			panic("region_alloc: page_alloc failed!");
		if (page_insert(e->env_pgdir, pp, (void *)start, PTE_U | PTE_W)) 
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	if (!e || !binary) {
		panic("load_icode: invalid arguments!");
	}
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	if (type  == ENV_TYPE_FS) {
		...
		env->env_tf.tf_eflags |= FL_IOPL_3; 	// highest IOPL
	}
}
